...
namespace structures
{
	...
	inline LinkedListItem<T>::LinkedListItem(T data):
		DataItem<T>(data), //dedi dataItem
		next_(nullptr) //dalsi smernik nastavim na nullptr
	{
	}

	template<typename T>
	inline LinkedListItem<T>::LinkedListItem(const LinkedListItem<T>& other):
		DataItem<T>(other), //zavolam kopirovaci kontruktor
		next_(other.next_)
	{
	}
	...
	inline LinkedList<T>::LinkedList(const LinkedList<T>& other):
		LinkedList()
	{
		*this = other; //zavolanie kontruktora, prazdneho zoznamu a zavolam prazdny prikaz
	}
	...
	template<typename T>
	inline LinkedList<T>& LinkedList<T>::operator=(const LinkedList<T>& other) //priradenie
	{
	...
	{
		LinkedListItem<T>* newItem = new LinkedListItem<T>(data); //vytvorim si novy linkedListItem
		if (size_ == 0) 
		{
			first_ = newItem;
			last_ = newItem;
		...
		{
			last_->setNext(newItem); // musim mat istotu, ze existuje
			last_ = newItem;
		}

		size_++;//zvysenie velkosti
	}
	...
	inline void LinkedList<T>::insert(const T & data, const int index)
	{
		...
		else
		{
			...
			{
				LinkedListItem<T>* prevItem = getItemAtIndex(index - 1);//prvok, ktory sa nachadza pred nim
	...
	inline T LinkedList<T>::removeAt(const int index)
	{
		...
		if (first_ == last_) //jedna sa o jediny prvok linklistu
		{
			delItem = first_;
			first_ = nullptr;
			last_ = nullptr;
		...
		else
		{
			if (index == 0)
			{
				delItem = first_; //ak je odoberany prvy prvok, tak si ho zapamatam a adresu posuniem
				first_ = first_->getNext();
			}
			else
			{
				LinkedListItem<T>* prevItem = getItemAtIndex(index - 1); //ak to nie je prvy prvok, musim si ho najprv najst. Najst predchadzajuci prvok, a najst nasledujuci vymazaneho
