...
namespace structures 
{
	...
	template<typename T>
	Array<T>::Array(const size_t size) : //konstruktor vector
	...
	template<typename T> //zavolanie kopirovacieho konstrukora zavolam sam seba
	Structure* Array<T>::clone() const
	{
		return new Array<T>(*this);
	}

	template<typename T> // other na pole 
	...
	{
		if (this != &other) //nepriradovanie sam do seba
		{
			if (size_ != other.size_) //osetrenie velkosti pola
			{
				throw std::invalid_argument("Arrays' sizes differ!");
			}
			*vector_ = *(other.vector_); //priradovaci prikaz na vectory
		}
		return *this; //vratim odkaz na seba
	}
	...
	{
		int vectorIndex = mapFunction(index); //zistenie, kde sa index nachadza, kde zacina vo vectore. Pouzijem mapovaciu funkciu
		byte* vectorPointer = vector_->getBytePointer(vectorIndex); //potrebujeme adresu zaciatku. Potrebujem sa tam dostat, zavolam getBytePointer a dostanem adresu ukazovatela
		T* typeVectorPointer = reinterpret_cast<T*>(vectorPointer); //pretypovanie aby to bol smernik na T = T*
		return *typeVectorPointer; //vratiti to, na co nam T ukazuje
	}

	template<typename T>
	const T Array<T>::operator[](const int index) const
	{
		return *reinterpret_cast<T*>(vector_->getBytePointer(mapFunction(index))); //to iste co vyssie len kratsie
	}

	template<typename T> //operator == 
	inline bool Array<T>::operator==(const Array<T>& other) const //ak sa rovnaju vectory, rovnaju sa aj velkosti
	{
		return *vector_ == *other.vector_; //nemusime porovnavat velkost
	}
	...
	void Array<T>::copy(const Array<T>& src, const int srcStartIndex, Array<T>& dest, const int destStartIndex, const int length)
	{
		if (length != 0)
		{
			Vector::copy(*src.vector_, 
				src.mapFunction(srcStartIndex), *dest.vector_, //kontrola indexov v mapovacej funkcii
	...
	{
		DSRoutines::rangeCheckExcept(index, size_, "Invalid index in array!"); // index, ktory sa pokusam pretypovat
		return index * sizeof(T); // mapovanie spravim prenasobenim 
	}
}
