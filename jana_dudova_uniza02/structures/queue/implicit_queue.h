...
namespace structures
{
	...
	ImplicitQueue<T>::ImplicitQueue() :
		ImplicitQueue(10) //parameter velkost pola
	{
	}
	...
	ImplicitQueue<T>::ImplicitQueue(const size_t capacity) :
		...
		startIndex_(0), //startovaci index
		size_(0) //velkost idexu
	{
	}
	...
	ImplicitQueue<T>::ImplicitQueue(const ImplicitQueue<T>& other) :
		...
		startIndex_(other.startIndex_),//startovaci index sa po vymazani automaticky posunie
		size_(other.size_)
	{
	}
	...
	inline void ImplicitQueue<T>::push(const T& data)
	{
		...
		if (size_ < capacity)
		{
			(*array_)[static_cast<int>((startIndex_ + size_) % capacity)] = data; // modulo kapacity ide dokola, ze zacina znova od prveho
			size_++;
		}
		else
		{
	...
	inline T ImplicitQueue<T>::pop()
	{
		if (size_ > 0)
		{
			...
			startIndex_ = ++startIndex_ % array_->size();//startovaci index pouzivam s modulom, aby mi nepretiekol za pole
			return pomData;
		}
		else
		{
