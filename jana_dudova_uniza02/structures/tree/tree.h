...
namespace structures
{
	...
	{
		TreeNode<T>* result = shallowCopy(); //najprv plytku kopiu, rechadzam cez vsetkych synov
	...
	{
		return parent_ == nullptr; //zistenie, ci je moj otec nullptr
	}
	...
	{
		if (!isRoot())//ci je koren
		{
			return parent_->getSon(brothersOrder);//ak existuje otec,poziadam ho aby mi dal syna na takejto pozicii
		}
		else
		{
	...
	{
		size_t result = 1; //majprv zapocitam seba a potom vsetko co je podo mnou
		for (int i = 0; i < degree(); i++) //zisti pocet synov
		{
			TreeNode<T>* son = getSon(i);
			if (son != nullptr)
			{
	...
	{
		return root_ == nullptr; //zlozitost O(1)
	}
	...
	inline Tree<T>& Tree<T>::operator=(const Tree<T>& other)
	{
		if (this != &other)
		{
			clear();//vycistit musim

			if (other.root_ != nullptr)
			{
				root_ = other.root_->deepCopy();//nahradim koren hlbokou kopiou korena
			}
		}
		return *this;
	}
	...
	{
		delete root_; //staci zmazat koren, ocakavam ze sa zmaze vsetko pod nim... rekurzia, postOrder, najprv synov potom seba
		root_ = nullptr;
	}
	...
	{
		TreeNode<T>* oldRoot = root_; //najdem stary koren
		root_ = newRoot;//nahradim novym
		return oldRoot;
	}
	...
	inline Tree<T>::TreeIterator::TreeIterator():
		Iterator<T>(),
		path_(new std::queue<TreeNode<T>*>()) //budu v takom poradi v akom ich mame prehliadat, lepsie pouzit implicitny front 
	{
	}
