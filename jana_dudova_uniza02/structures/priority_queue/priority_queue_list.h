...
namespace structures
{
	...
	inline PriorityQueueList<T>::PriorityQueueList(const PriorityQueueList<T>& other) :
		PriorityQueueList<T>(dynamic_cast<List<PriorityQueueItem<T>*>*>(other.list_->clone())) //kopirovaci kontruktor, kopiu zoznamu ziskam pomocou operacie klonovania
	{
		list_->clear();
		*this = other;
	}
	...
	inline PriorityQueueList<T>& PriorityQueueList<T>::operator=(const PriorityQueueList<T>& other)
	{
		if (this != &other)
		{
			...
			{
				list_->add(new PriorityQueueItem<T>(*item)); //tu si vytvaram kopiu jednotlivych itemov
			}
		}
		return *this;
	}
	...
	{
		for (PriorityQueueItem<T>* item : *list_) //prechadzanie zoznamu pomocou iteratora
		{
			delete item;//musim zlikvidovat cervene itemy
		}
		list_->clear();
	}
	...
	inline int PriorityQueueList<T>::indexOfPeek() const
	{
		...
		int index = i;
		for (PriorityQueueItem<T>* item : *list_) //prehladavam cyklom a ak najdem nieco lepsie tak si to zapamatam, aj kde som to nasiel aj index. zlozitost je O(n)
		{
